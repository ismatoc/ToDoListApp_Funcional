    ...
    return LayoutBuilder(
      builder: (_, c) {
        ...
        return GridView.builder(
          ...
          itemBuilder: (_, i) {
            final url = urls[i];
            return VideoThumbTileVP(url: url);
          },
        );
      },
    );
...
class VideoThumbTileVP extends StatefulWidget {
  final String url;
  const VideoThumbTileVP({super.key, required this.url});
      ...
      child: GestureDetector(
        onTap: () {
          showDialog(
            context: context,
            barrierColor: Colors.black87,
            builder: (_) => VideoPlayerDialogVP(url: widget.url),
          );
        },
        child: Stack(
          fit: StackFit.expand,
...
class VideoPlayerDialogVP extends StatefulWidget {
  final String url;
  const VideoPlayerDialogVP({super.key, required this.url});
